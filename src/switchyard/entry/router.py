...
class Router:
    def __init__(self, app: YardApplication):
        ...
        self.routes = [
            ...
            Route("POST", r"/api/intake-trains/(?P<code>[^/]+)/correct", self._correct_intake),
            Route("GET", r"/api/intake-trains/(?P<code>[^/]+)/manifest", self._intake_manifest),
            Route("GET", r"/api/intake-trains/(?P<code>[^/]+)/manifest-versions", self._intake_manifest_versions),
    ...
    def _correct_intake(self, body: Any, code: str) -> dict[str, Any]:
        return intake_service.correct_intake_manifest(self.app, code, body)

    def _intake_manifest(self, body: Any, code: str) -> dict[str, Any]:
        return query_service.intake_manifest_view(self.app, code)

    def _intake_manifest_versions(self, body: Any, code: str) -> dict[str, Any]:
        return query_service.intake_manifest_versions(self.app, code)
